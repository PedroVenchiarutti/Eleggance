...
const getBodyObjects = (ordersList, orderBy) =>
...
const getProductsQuantity = (products) =>
  products.map((product) => product.qt_product).reduce(sum, 0);
...
const getOrderPrice = (products) =>
  parseFloat(
    products
      .map((product) => product.value * product.qt_product)
      .reduce(sum, 0)
      .toFixed(2)
  )
    .toString()
